...
def write_run_batch_file(
    ...
    with open(script_file, "w") as fh:
      ...
      # fh.writelines(f"#SBATCH --mem={numeric_settings['max_memory']}\n")
        ...
        fh.writelines("module load openmpi/4.1.5\n")
        fh.writelines("module load gcc/12.2.0\n")
        fh.writelines("module load hdf5/1.14.1-2_openmpi-4.1.5_parallel\n")
        fh.writelines("module load cuda/12.2\n\n")
        ...
        fh.writelines(f"MODEL_DIR={out_dir}\n")
        fh.writelines("OUT_DIR=outputs/${MODEL_DIR}\n\n")
...
def write_analysis_batch_file(
    ...
    with open("submit_analysis.sh", "w") as fa:
        ...
        fa.writelines(f"MODEL_DIR={out_dir}\n")
        fa.writelines("OUT_DIR=outputs/${MODEL_DIR}\n")
